...
class Tank:
    ...
    def bullet_move(self, map1, enemy_rect):
        if self.bullet is not None:
            self.bullet.move(map1, enemy_rect)
            if self.bullet.end_life:
                self.bullet = None

    def move(self, map2, enemy_rect):
        self.direction = 0
        if not self.spin:
        ...
        self.colliding_rects(map2 + [enemy_rect])
        self.bullet_move(map2, enemy_rect)

        if self.start_spin > 200:
            self.spin = False
            self.random_pos(map2)
            self.start_spin = 0
        if self.spin:
            self.start_spin += 1
            self.angle += 22.5
